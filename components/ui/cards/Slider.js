...
// const Slider = ({ category, tags, title, isFeatured }) => {
//   const { t } = useTranslation();
//   const [videos, setVideos] = useState([]);
//   const { user } = useContext(authContext);
//   const videoFeatch = useVideoFetch();

//   useEffect(() => {
//     const fetchData = async () => {
//       try {
//         const { data } = await videoFeatch(category, tags, isFeatured);
//         setVideos(data);
//       } catch (error) {
...
//               className="relativ group overflow-hidden rounded-xl"
//             >
//               <Link href={`/video/${video?._id}`}>
//                 <Image
//                   className="w-[350px] md:w-[300px] h-[350px] transform transition-transform duration-300 group-hover:scale-110 group-hover:overflow-hidden"
//                   src={video?.thambnail}
//                   width={350}
//                   height={350}
//                   alt={video?.title}
...
const Slider = ({ category, tags, title, isFeatured }) => {
  ...
  const [videos, setVideos] = useState([]);
  const { user } = useContext(authContext);
  const videoFetch = useVideoFetch(category, tags, isFeatured); // Pass parameters directly to the hook

  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await videoFetch; // Use videoFetch directly
        setVideos(data);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    ...
    fetchData();
  }, [videoFetch]); // Adjust dependency array if needed
