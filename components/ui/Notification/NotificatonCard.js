    ...
    return (
        <div>
            <motion.div
                    ...
                    >
        <Link href={`/${notification?.category}/${notification?.contentId}`}>
            ...
            <IoIosNotifications className="text-gray-100 text-3xl px-1 py-1 rounded-lg bg-[#ee561c]" />
                <div>
                    ...
                    <h1 className='text-left text-[10px] md:text-xs  md:my-[3px] '>Relis: {new Date(notification?.date).toLocaleDateString()} </h1>
                </div>
            </div>
        </Link>
        </motion.div>
