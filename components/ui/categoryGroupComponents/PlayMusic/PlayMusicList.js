...
const PlayMusicList = ({ category}) => {

    const [sadMusics, setSadMusics] = useState([]);
    const [loading, setLoading] = useState(true);
  
    useEffect(() => {
      axios
        .get(`https://vibewabe-server.vercel.app/music?category=sad&sortBy=view&sort=-1&limit=20`)
        .then((res) => {
            setSadMusics(res.data);
          setLoading(false);
        })
        .catch((error) => {
