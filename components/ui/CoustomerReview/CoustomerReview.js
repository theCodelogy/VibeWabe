...
const CoustomerReview = ({ id }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://vibewabe-server.vercel.app/comment/${id}?category=video`)
      .then((res) => {
        setComment(res.data);
      });
  }, [id]);
  ...
  const handle = async (event) => {
    ...
    axios
      .post("https://vibewabe-server.vercel.app/comment", allData)
      .then((res) => {
  ...
  return (
    ...
    <div className="  rounded-lg  p-2 my-4 mx-6">
      ...
      <div className="flex  flex-col h-64 overflow-y-auto">
        {comments.map((data) => (
          <div key={data.videoId} className="border rounded-md p-3 ml-3 my-3 ">
